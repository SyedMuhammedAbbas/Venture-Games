...
import xoxoCover from "../../images/sliderCover/xoxoCover.jpg";
...
export default function TopGames() {
  ...
  //   }, []);
  const CoverSlider = [
    { LapImage: SpidermanCover.src, MobImage: xoxoCover.src },
    { LapImage: Cover.src, MobImage: xoxoCover.src },
  ];
  ...
  return (
    ...
    <div className="flex min-w-[100vw] max-w-[100%] h-[100%]">
      <Swiper
        ...
        className="mySwiper"
      >
        {Object.values(CoverSlider).map((currentItem) => (
          <SwiperSlide>
            <TopGamesSlide
              key={currentItem}
              LaptopcarouselProd={currentItem.LapImage}
              MobilecarouselProd={currentItem.MobImage}
            />
          </SwiperSlide>
        ))}
      </Swiper>
